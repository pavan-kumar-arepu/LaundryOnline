...
class TicketModel: Object, ObjectKeyIdentifiable {
...
extension TicketModel {
    static var areTicketsPopulated: Bool {
        do {
            let realm = try Realm()
            let ticketObjects = realm.objects(TicketModel.self)
    ...
    static func bootstrapTickets() {
        do {
            let realm = try Realm()
            let ticket1 = TicketModel(reportedBy: "Andrew", product: "Realm", title: "Problem 1", problemDescription: "This is the first problem")
            let ticket2 = TicketModel(reportedBy: "Rod", product: "Realm", title: "Problem 2")
            let ticket3 = TicketModel(reportedBy: "Jane", product: "Realm", title: "Problem 3", problemDescription: "This is the third problem. It should have a longer description that I'd expect to see wrap over multiple lines in the UI")
            ticket1.status = .complete
            ticket3.status = .inProgress
            try realm.write {
                realm.delete(realm.objects(TicketModel.self))
                realm.add(ticket1)
                realm.add(ticket2)
                realm.add(ticket3)
            }
